...
@export var tree_density: float = 0.4
@export var bush_density: float = 1.2
...
@export var tree_collision_enabled: bool = true
@export var tree_collision_radius: float = 0.05
@export var tree_collision_height: float = 8.0

@export_group("Clustering")
@export var use_clustering: bool = true
@export var clusters_per_chunk: int = 5
@export var vegetation_per_cluster: int = 12
@export var cluster_radius: float = 8.0
@export var tree_to_bush_fade: bool = true  # Trees in center, bushes at edges
...
func spawn_clustered_vegetation(chunk_pos: Vector2, chunk_vegetation: Array, rng: RandomNumberGenerator, is_preview: bool):
	...
	for cluster_i in range(clusters_per_chunk):
		...
		# Decide cluster type: tree cluster, bush cluster, or mixed
		var cluster_type = rng.randf()

		if cluster_type < 0.4:
			# Tree cluster with bushes fading at edges
			spawn_tree_cluster(chunk_pos, chunk_vegetation, rng, cluster_center_x, cluster_center_z)
		elif cluster_type < 0.7:
			# Bush cluster
			spawn_bush_cluster(chunk_pos, chunk_vegetation, rng, cluster_center_x, cluster_center_z)
		else:
			# Mixed cluster (original behavior)
			spawn_mixed_cluster(chunk_pos, chunk_vegetation, rng, cluster_center_x, cluster_center_z)

func spawn_tree_cluster(chunk_pos: Vector2, chunk_vegetation: Array, rng: RandomNumberGenerator, center_x: float, center_z: float):
	var total_count = int(vegetation_per_cluster * (tree_density + bush_density))
	var tree_zone = cluster_radius * 0.5  # Trees only in inner 50%

	for i in range(total_count):
		var distance_from_center = rng.randf_range(0, cluster_radius)
		var angle = rng.randf_range(0, TAU)
		var veg_x = center_x + cos(angle) * distance_from_center
		var veg_z = center_z + sin(angle) * distance_from_center

		# Trees in center, bushes at edges
		var normalized_distance = distance_from_center / cluster_radius
		var tree_chance = 1.0 - normalized_distance  # 100% tree at center, 0% at edge

		var veg_type: VegetationType
		if tree_to_bush_fade:
			veg_type = VegetationType.TREE if rng.randf() < tree_chance else VegetationType.BUSH
		else:
			veg_type = VegetationType.TREE if distance_from_center < tree_zone else VegetationType.BUSH

		var mesh_instance = create_vegetation_instance(veg_type, rng)
		position_vegetation(mesh_instance, chunk_pos, veg_x, veg_z, distance_from_center, rng, veg_type)
		add_child(mesh_instance)
		chunk_vegetation.append(mesh_instance)

func spawn_bush_cluster(chunk_pos: Vector2, chunk_vegetation: Array, rng: RandomNumberGenerator, center_x: float, center_z: float):
	var bush_count = int(vegetation_per_cluster * bush_density * 1.5)  # Extra dense bush clusters

	for i in range(bush_count):
		var distance_from_center = rng.randf_range(0, cluster_radius * 0.8)  # Tighter clusters
		var angle = rng.randf_range(0, TAU)
		var veg_x = center_x + cos(angle) * distance_from_center
		var veg_z = center_z + sin(angle) * distance_from_center
...
func spawn_mixed_cluster(chunk_pos: Vector2, chunk_vegetation: Array, rng: RandomNumberGenerator, center_x: float, center_z: float):
...
func create_vegetation_instance(type: VegetationType, rng: RandomNumberGenerator) -> Node3D:
	...
	if type == VegetationType.TREE and tree_collision_enabled:
		var root = Node3D.new()
		...
		cylinder.radius = tree_collision_radius
		cylinder.height = tree_collision_height
		collision_shape.shape = cylinder
		collision_shape.position.y = tree_collision_height * 0.5
...
func position_vegetation(node: Node3D, chunk_pos: Vector2, local_x: float, local_z: float, distance_from_center: float, rng: RandomNumberGenerator, type: VegetationType):
	var chunk_size = terrain_system.chunk_size if terrain_system else 32
	...
	node.position = Vector3(world_x, height + height_offset, world_z)
	...
	for chunk_key in spawned_vegetation.keys():
		...
		for veg in chunk_veg:
			if not is_instance_valid(veg):
				continue
			# Check if it's a tree (Node3D with MeshInstance3D child) or bush (MeshInstance3D)
			var mesh: MeshInstance3D = null
			if veg is MeshInstance3D:
				mesh = veg
			elif veg.get_child_count() > 0 and veg.get_child(0) is MeshInstance3D:
				mesh = veg.get_child(0)

			if mesh and mesh.material_override == tree_material:
				total_trees += 1
			elif mesh and mesh.material_override == bush_material:
				total_bushes += 1
